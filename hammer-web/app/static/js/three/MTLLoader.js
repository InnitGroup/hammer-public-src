...
// RBX: Override map_fragment to blend diffuse map with base part color.
...
// RBX: Override roughnessmap_fragment to source roughness from green channel. Clamp it the same way we do in our engine.
...
// RBX: Override roughnessmap_fragment to source metalness from red channel.
	...
	createMaterial_( materialName ) {
		...
		function setMapForType( mapType, value ) {
			...
			if ( mapType === 'map' || mapType === 'emissiveMap' ) {
				...
				// RBX: No sRGB encoding??
				//map.colorSpace = SRGBColorSpace;
			}

			// RBX: Assing same texture to both metalnessMap and roughnessMap
		...
		for ( const prop in mat ) {
			...
			switch ( prop.toLowerCase() ) {
				...
				case 'kd':
					...
					// Diffuse color (color under white light) using RGB values
					// RBX: OBJ color is already in linear space.
				...
				case 'bump':
					...
					// Bump texture map
					// RBX: We interpret map_bump as a normal map.
					setMapForType('normalMap', value);
				...
				case 'ns':
					...
					// RBX: Does not make sense with PBR material.
		...
		// RBX: default to non-metal, fully diffuse.
		params.metalness = 0;
		params.roughness = 1;
  
		// RBX: Using PBR material.
