"""
    ...
    Note: Technically this does not need to be ported
    to async code as the Database Driver is still synchronous.
"""

import logging
import redis_lock
from app.extensions import db, sync_redis_controller
from app.services.authentication import get_user_by_id
...
def GetGroupIconFromGroup( TargetGroup : Group | int ) -> GroupIcon | None:
    """
    Returns a GroupIcon object from a Group.
    """
    TargetGroup : Group = GetGroupFromId(TargetGroup)

    return GroupIcon.query.filter_by(group_id=TargetGroup.id).first()
...
def GetGroupMemberCount( TargetGroup : Group | int ) -> int:
    """
        Returns the amount of members in a group.
    """
    TargetGroup : Group | None = GetGroupFromId(TargetGroup)
    TotalMembers : int = 0
    for Role in GroupRole.query.filter_by(group_id=TargetGroup.id).all():
...
def GetUserRankInGroup( TargetUser : User | int, TargetGroup : Group | int ) -> int:
    """
        ...
        255 - Group Owner
    """
    TargetUser : User = get_user_by_id(TargetUser)
    TargetGroup : Group = GetGroupFromId(TargetGroup)
...
from app.services.text_moderation import filter_text, TextNotAllowed
...
def isGroupNameAllowed( GroupName : str ) -> bool:
    ...
    if AlphanumericCharacters < 3:
        return False
    try:
        filter_text( Text = GroupName, ThrowException = True)
    except TextNotAllowed:
        return False
    return True
...
def CreateGroup( GroupName : str, GroupDescription : str, GroupOwner : User | int, GroupIconContentHash : str ) -> Group | None :
    ...
    with redis_lock.Lock( redis_client = sync_redis_controller, name = "group_creation_lock", expire = 10, auto_renewal = True ) as CreateLock:
        if SearchGroupByName(GroupName) is not None:
            redis_lock.release_lock("group_creation_lock", CreateLock)
        ...
        GroupOwner : User = get_user_by_id(GroupOwner)
...
def PostToGroupStatus( Poster : User | int, TargetGroup : Group | int, StatusMessage : str, ForcePost : bool = False ) -> GroupStatus:
    ...
    """
    Poster : User = get_user_by_id(Poster)
    TargetGroup : Group = GetGroupFromId(TargetGroup)
    if not ForcePost:
        AssertUserHasPermission(Poster, TargetGroup, GroupRolePermission.post_to_status)
    
    FilteredMessage : str = filter_text(Text=StatusMessage)
...
def PostToGroupWall( Poster : User | int, TargetGroup : Group | int, WallMessage : str, ForcePost : bool = False ) -> GroupWallPost:
    ...
    FilteredMessage : str = filter_text(Text=WallMessage)
    if len(FilteredMessage) > 1024:
...
def DeleteGroupWallPost( TargetPost : GroupWallPost, Remover : User | int, ForceDelete : bool = False ) -> None:
    ...
    Remover : User = get_user_by_id(Remover)
...
def SetNewGroupIcon( TargetGroup : Group | int, NewIconHash : str, Uploader : User | int ) -> GroupIcon:
    ...
    """
    TargetGroup : Group = GetGroupFromId(TargetGroup)
    Uploader : User = get_user_by_id(Uploader)
...
def GetUserGroupCount( TargetUser : User | int ) -> int:
    """
        Gets the amount of groups a user is in.
    """
    TargetUser : User = get_user_by_id(User)

    return GroupMember.query.filter_by( user_id = TargetUser.id ).count()